...
const shortestPaths = universe => {
  ...
  for(let i = 0; i < universe.galaxies.length - 1; i++) {
    ...
    for(let j = i + 1; j < universe.galaxies.length; j++) {
      ...
      dist += (1000000 - 1) * universe.yExpansions.filter(y => isInRange(toRange(a.y, b.y), y)).length;
      dist += (1000000 - 1) * universe.xExpansions.filter(x => isInRange(toRange(a.x, b.x), x)).length;
      res.push(dist);
    }
  }
