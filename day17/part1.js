...
const pathFind = grid => {
  ...
  const minPath = aStar(
    { pos: startPos, cost: 0, dir: [] },
    x => equals(x.pos, factoryPos),
    x => getCardinalNeighbors(grid, x.pos)
      .map(y => ({ pos: y, prev: x, cost: x.cost + getValue(grid, y), dir: [sub(y, x.pos), ...R.take(3, x.dir)]}))
      .filter(y => y.dir.length < 2 || !equals({x: 0, y: 0}, add(y.dir[0], y.dir[1])))
      .filter(y => y.dir.length < 4 || !allEqual(y.dir, equals)),
    x => x.cost,
    x => manhattan(sub(x.pos, factoryPos)),
    x => `${toString(x.pos)}:${x.dir.map(toString).join(',')}`
  );
  //print(grid, minPath);
  return minPath.cost;
};
